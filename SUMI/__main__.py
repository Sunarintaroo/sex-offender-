...
def start(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_video(
            video="https://te.legra.ph/file/c6105e87c2a986f412299.mp4",
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            ...
                            InlineKeyboardButton(
                                text="⬅ ʜᴏᴍᴇ", callback_data="sumi_back"
                            ),
                        ]
                    ]
                ),
...
def repo_callback_data(update, context):
    ...
    if query.data == "repo_":
        ...
        first_name = update.effective_user.first_name
        user_name = update.effective_user.username
        query.message.edit_caption("Test")
...
def get_help(update: Update, context: CallbackContext):
    ...
    if update.effective_chat.type != update.effective_chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            ...
            first_name = update.effective_user.full_name
            update.effective_message.reply_photo(
                random.choice(SUMI_N_IMG),
                caption=f"Hey {first_name}, Click the Button Below to get help of {module.capitalize()}",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
        ...
        update.effective_message.reply_photo(
            random.choice(SUMI_N_IMG),
            caption="Hey [{}](tg://user?id={}) Click the Button Below to get the list of possible commands.".format(
...
def settings_button(update: Update, context: CallbackContext):
    ...
    try:
        if mod_match:
            ...
            query.message.reply_photo(
                photo=random.choice(PM_PHOTO),
                caption="Hi there! There are quite a few settings for {} - go ahead and pick what you're interested in.".format(
                    chat.title
                ),
                reply_markup=InlineKeyboardMarkup(
                    paginate_modules(
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        try:
            ...
            update.effective_message.reply_text("I've PM'ed you about donating!")
        except Unauthorized:
            update.effective_message.reply_text(
...
def main():
    if SUPPORT_CHAT is not None and isinstance(SUPPORT_CHAT, str):
        try:
            ...
            m = dispatcher.bot.send_photo(
                f"@{SUPPORT_CHAT}",
                SUMI_DISPACHER_PIC,
                caption=f"ᏦϴΝΝᏆᏟᎻᏆᏔᎪ !! (◍•ᴗ•◍)",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton(text="Assist", url="https://t.me/itz_Erwin_Smith")]]
                ),
            )
        except Unauthorized:
            LOGGER.warning(
